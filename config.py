...
SHOT_CLOCK_WARN_SECONDS = 10         # 剩余低于此秒数时 HUD 数字变红预警 + 每秒滴答
COLOR_SHOT_CLOCK = (235, 235, 235)   # 计时器正常色
COLOR_SHOT_CLOCK_WARN = (230, 60, 60)  # 计时器预警色（剩余不足时）
# 预警/超时合成音（代码合成，无音频文件）
SHOT_CLOCK_TICK_FREQ = 880           # 每秒滴答音高（Hz，清脆短音）
SHOT_CLOCK_TICK_MS = 60              # 滴答时长（毫秒）
SHOT_CLOCK_TIMEOUT_FREQ = 330        # 超时音高（Hz，较低，区别于滴答）
SHOT_CLOCK_TIMEOUT_MS = 220          # 超时音时长（毫秒）
SHOT_CLOCK_BEEP_VOLUME = 0.4         # 滴答/超时音量 [0,1]
